...
//#include <play_guilds>
#include <play_stamina>

#define PLUGIN "Zombie Plague"
#define VERSION "4.0"

#define ZP_CUSTOMIZATION_FILE "tsw_zombiexp/tsw_zmxp_manager.ini"
#define ZP_CONFIGS_FILE "tsw_zombiexp/tsw_zmxp_manager.cfg"
#define ZP_EXTRAITEMS_FILE "tsw_zombiexp/tsw_zmxp_extraitems.ini"
#define ZP_ZOMBIECLASSES_FILE "tsw_zombiexp/tsw_zmxp_classes.ini"
...
new xPlayerFlareColor[ 33 ];
new xColorOwner[ MAX_COLORS ];

new const xColorCode[ 10 ][] = {
	{255, 0, 0},
	{255, 0, 204},
	{204, 0, 255},
	{108, 0, 255},
...
new const WEAPONNAMES[][] = { "", "P228 Compact", "", "Schmidt Scout", "", "XM1014 M4", "", "Ingram MAC-10", "Steyr AUG A1",
			...
			"M3 Super 90", "M4A1 (Colt)", "Schmidt TMP", "G3SG1 Auto-Sniper", "", "Desert Eagle .50 AE",
...
/* Efeito de gelo na frost */
//new Glowx
new Float:g_velocidade[33] // velocidade do upgrade (cache)
new Float:g_gravity_human[33] // gravidade do upgrade (cache)
new Float:g_gravity_zm[33] // gravidade do upgrade (cache)
new g_hp[33] // hp do upgrade (cache)
...
new g_hasleap[33] // native de leap para zm =D
new g_congelante[33] // native do congelante
new g_flamejante[33] // native do flamejante
...
new g_hasmadness[33] // zombie madness editado
new g_respawn_as_zombie[33] // should respawn as zombie
new g_nvision[33], Float:g_lastnvg[33] // has night vision
...
new xSpriteInfectTrail, xSpriteInfectGib, xSpriteInfectGib2, xSpriteInfectExp;
new xSpriteFireTrail, xSpriteFireGib, xSpriteFireGib2, xSpriteFireExp, xSpriteFireSmoke;
new xSpriteFrostTrail, xSpriteFrostGib, xSpriteFrostGib2, xSpriteFrostExp;

new xSpriteFlareTrail;

new g_exploSpr, g_flameSpr, g_smokeSpr, g_glassSpr

new xSpriteInfectEffect
...
new g_infbombcounter, xBuyInfectBomb, g_antidotecounter, g_madnesscounter // to limit buying some items
...
model_grenade_fire[64], model_grenade_frost[64], model_grenade_flare[64], w_model_grenade_flare[64],
model_vknife_admin_human[64], model_vknife_admin_zombie[64]

new xLoadSpriteInfectTrail[ 64 ], xLoadSpriteInfectGib[ 64 ], xLoadSpriteInfectGib2[ 64 ], xLoadSpriteInfectExp[ 64 ], xLoadSpriteInfectSmoke[ 64 ];
new xLoadSpriteFireTrail[ 64 ], xLoadSpriteFireGib[ 64 ], xLoadSpriteFireGib2[ 64 ], xLoadSpriteFireExp[ 64 ], xLoadSpriteFireSmoke[ 64 ];
new xLoadSpriteFrostTrail[ 64 ], xLoadSpriteFrostGib[ 64 ], xLoadSpriteFrostGib2[ 64 ], xLoadSpriteFrostExp[ 64 ], xLoadSpriteFrostSmoke[ 64 ];
new xLoadSpriteFlareTrail[ 64 ], xLoadSpriteFlareGlow[ 64 ];

new xLoadSpriteInfectEffect[ 64 ]

new sprite_grenade_ring[64], sprite_grenade_fire[64],
...
//new cvar_infbomblimit;

new cvar_allowrespawnsurv, cvar_flashshowall, cvar_allowrespawninfection, cvar_allowrespawnnem,
...
// Cached CVARs
new g_cached_customflash, g_cached_zombiesilent, g_cached_leapzombies, Float:g_cached_leapzombiescooldown, g_cached_leapnemesis,
...
//native get_user_personagem( index, team );
//native zmxp_get_humman_model( index, model[], len );

//new xModelHumman[ 33 ][ 32 ];

/** EFEITO DAS GRANADAS **/
new Float: inf_time[ 33 ], Float: inf_dmgtime[ 33 ], Float: inf_touch[ 33 ], Float:fsmoke_touching[33]

#define SMOKE_INFECT 5612
#define SMOKE_FROST 8751

new temp_entity[1300]
new Float:f_unfreeze_time[33]
...
public plugin_natives()
{
	...
	register_native("zp_set_user_leap", "native_set_user_leap", 1)
	register_native("zp_user_has_leap", "native_user_has_leap", 1)
	register_native("zp_give_user_madness", "native_give_user_madness", 1)
	register_native("zp_get_user_model", "native_get_user_model", 0)
	register_native("zp_set_user_model", "native_set_user_model", 1)
	register_native("zp_set_zombie_congelante", "native_set_zombie_congelante", 1)
	register_native("zp_set_zombie_flamejante", "native_set_zombie_flamejante", 1)
...
public plugin_precache(){
	...
	engfunc(EngFunc_PrecacheModel, w_model_grenade_flare)
	engfunc(EngFunc_PrecacheModel, model_vknife_admin_human)
	engfunc(EngFunc_PrecacheModel, model_vknife_admin_zombie)
	
	precache_generic( "de_vegas.wad" ) 
	
	// Custom sprites for grenades
	xSpriteInfectTrail = engfunc( EngFunc_PrecacheModel, xLoadSpriteInfectTrail );
	xSpriteInfectGib = engfunc( EngFunc_PrecacheModel, xLoadSpriteInfectGib );
	xSpriteInfectGib2 = engfunc( EngFunc_PrecacheModel, xLoadSpriteInfectGib2 );
	xSpriteInfectExp = engfunc( EngFunc_PrecacheModel, xLoadSpriteInfectExp );
	engfunc( EngFunc_PrecacheModel, xLoadSpriteInfectSmoke );
	
	xSpriteFireTrail = engfunc( EngFunc_PrecacheModel, xLoadSpriteFireTrail );
	xSpriteFireGib = engfunc( EngFunc_PrecacheModel, xLoadSpriteFireGib );
	xSpriteFireGib2 = engfunc( EngFunc_PrecacheModel, xLoadSpriteFireGib2 );
	xSpriteFireExp = engfunc( EngFunc_PrecacheModel, xLoadSpriteFireExp );
	xSpriteFireSmoke = engfunc( EngFunc_PrecacheModel, xLoadSpriteFireSmoke );

	xSpriteFrostTrail = engfunc( EngFunc_PrecacheModel, xLoadSpriteFrostTrail );
	xSpriteFrostGib = engfunc( EngFunc_PrecacheModel, xLoadSpriteFrostGib );
	xSpriteFrostGib2 = engfunc( EngFunc_PrecacheModel, xLoadSpriteFrostGib2 );
	xSpriteFrostExp = engfunc( EngFunc_PrecacheModel, xLoadSpriteFrostExp );
	engfunc( EngFunc_PrecacheModel, xLoadSpriteFrostSmoke );
	
	xSpriteFlareTrail = engfunc(EngFunc_PrecacheModel, xLoadSpriteFlareTrail );
	engfunc(EngFunc_PrecacheModel, xLoadSpriteFlareGlow );
	...
	xSpriteInfectEffect = engfunc(EngFunc_PrecacheModel, xLoadSpriteInfectEffect)
...
public plugin_init()
{
	...
	// Language files
	register_dictionary_colored("tsw_zombie_plague.txt")
	...
	// granadas
	RegisterHam(Ham_Touch, "env_sprite", "fsmoke_touch");
	RegisterHam(Ham_Think, "env_sprite", "zn_smoke_think")
	RegisterHam(Ham_Think, "info_target", "infotarget_think")
	RegisterHam(Ham_Think, "info_target", "glow_effectz")
	...
	//register_clcmd("say /fantasias", "menu_roupas")
	//register_clcmd("say /fantasia", "menu_roupas")
	...
	cvar_ammoinfect = register_cvar("zp_zombie_infect_reward", "2")
...
public plugin_cfg(){
	...
	server_cmd("exec %s/%s", cfgdir, ZP_CONFIGS_FILE )
...
public fw_PlayerSpawn_Post(id){
	...
	g_lastnvg[id] = get_gametime()
	
	//xPlayerFlareColor[ id ] = 0;
	...
	if (g_respawn_as_zombie[id] && !g_newround)
	{
		...
		g_velocidade[id] = zmxp_get_user_speed(id) * 8.0	
		g_hp[id] = (zmxp_get_user_hp(id) * 40)
		switch(zmxp_get_user_gravity(id))
		{
			case 1:
			{
				g_gravity_human[id] = 0.95
				g_gravity_zm[id] = 0.03
			}
			case 2:
			{
				g_gravity_human[id] = 0.90
				g_gravity_zm[id] = 0.06
			}
			case 3:
			{
				g_gravity_human[id] = 0.86
				g_gravity_zm[id] = 0.09
			}
			case 4:
			{
				g_gravity_human[id] = 0.81
				g_gravity_zm[id] = 0.12
			}
			case 5:
			{
				g_gravity_human[id] = 0.76
				g_gravity_zm[id] = 0.15
			}
			default:
			{
				g_gravity_human[id] = 1.0
				g_gravity_zm[id] = 0.0
			}
		}		
	...
	fm_set_user_health2(id, get_pcvar_num(cvar_humanhp) + g_hp[id])
	set_pev(id, pev_gravity, g_gravity_human[id])
	...
	if (g_handle_models_on_separate_ent){
		...
		if( get_user_personagem( id, ZMXP_TEAM_HUMANO ) > 0 ){
			zmxp_get_humman_model( id, xModelHumman[ id ], charsmax( xModelHumman[]))
			
			fm_cs_set_user_model_index(id, 0)
			copy(g_playermodel[id], charsmax(g_playermodel[]), xModelHumman[ id ])
		}
		...
		*******************************/
		...
		/** SE ATIVAR OS PERSONAGENS DELETAR ABAIXO ***********************************************************************/
		iRand = random_num(0, ArraySize(model_human) - 1)
		ArrayGetString(model_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
		if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_human, iRand))
		/******************************************************************************************************************/
		...
		if( get_user_personagem( id, ZMXP_TEAM_HUMANO ) > 0 ){
			...
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, 0)
			copy(g_playermodel[id], charsmax(g_playermodel[]), xModelHumman[ id ])
		}
...
public fw_TraceAttack(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type)
{
	...
	// Tirar Knockback para PainShock Free
	if (g_painshockfree[victim])
		return HAM_IGNORED;		
...
public fw_SetModel(entity, const model[]){
	...
	if( model[9] == 's' && model[10] == 'm' ){
		static iClassName[ 10 ];
		pev(entity, pev_classname, iClassName, charsmax( iClassName ))
		if(equal( iClassName, "weaponbox") || equal( iClassName, "armoury_entity") || equal( iClassName, "grenade"))
		{
			entity_set_model(entity, w_model_grenade_flare ) // ENGINE
			return FMRES_SUPERCEDE
		}
	}
	*/
	...
	if(g_zombie[pev(entity, pev_owner)]){
		if(model[9] == 'h' && model[10] == 'e' && get_pcvar_num(cvar_extrainfbomb)){
			fm_set_rendering(entity, kRenderFxGlowShell, 0, 200, 0, kRenderNormal, 16);
			
			UTIL_BeamFollow( entity, xSpriteInfectTrail, 12, 40, 0, 255, 0, 255 );
			UTIL_BeamFollow( entity, xSpriteInfectTrail, 8, 30, 0, 255, 0, 150 );
			UTIL_BeamFollow( entity, xSpriteInfectTrail, 5, 5, 0, 255, 0, 255 );
	...
	// GRANADA DE FOGO ************************
	else if (model[9] == 'h' && model[10] == 'e' && get_pcvar_num( cvar_firegrenades )){
		fm_set_rendering(entity, kRenderFxGlowShell, 200, 0, 0, kRenderNormal, 16);
		
		UTIL_BeamFollow( entity, xSpriteFireTrail, 12, 40, 255, 128, 0, 255 );
		UTIL_BeamFollow( entity, xSpriteFireTrail, 8, 30, 255, 135, 40, 150 );
		UTIL_BeamFollow( entity, xSpriteFireTrail, 5, 5, 255, 255, 255, 255 );
	...
	// GRANADA DE GELO ***********************
		...
		UTIL_BeamFollow( entity, xSpriteFrostTrail, 20, 10, 0, 128, 255, 255 );
		
		set_pev(entity, PEV_NADE_TYPE, NADE_TYPE_FROST)
	}
	
	// GRANADA DE LUZ ************************
	else if( model[9] == 's' && model[10] == 'm' && get_pcvar_num( cvar_flaregrenades )){
		static rgb[3]
		...
		UTIL_BeamFollow( entity, xSpriteFlareTrail, 35, 10, rgb[0], rgb[1], rgb[2], 255 );
...
stock UTIL_BeamFollow( iEnt, iSprite, iLife, iWidth, iRed, iGreen, iBlue, iBright ){
...
public fw_ThinkGrenade(entity)
{
	...
	switch( pev( entity, PEV_NADE_TYPE )){
		case NADE_TYPE_INFECTION:{
			return infection_explode( entity );
		}
		...
		case NADE_TYPE_FLARE:{
			...
			if(( pev( entity, pev_flags ) & FL_ONGROUND ) && fm_get_speed( entity ) < 10 ){	
				static Float:o[3], rgb[3]
				pev(entity, PEV_FLARE_COLOR, rgb);
				pev( entity, pev_origin, o )
				set_pev(entity, pev_dmgtime, current_time + 99999.0)
				return zn_create_flare( pev( entity, pev_owner ), entity, o, rgb[0], rgb[1], rgb[2] )
			}
			
			else set_pev(entity, pev_dmgtime, current_time + 0.5)
		}
		...
		case NADE_TYPE_FLARE: // Flare
		{
			...
			if (duration > 0)
			{
				...
				//flare_lighting(entity, duration) // backup padrao zp
...
stock zn_create_flare( id, ent2, Float:o2[3], r, g, b )
{
	...
	//z_last_flare[id] = ent2
	new ent, Float:o[3], rgb[3], rgb2[3];
	o[0] = o2[0]
	o[1] = o2[1] + 4.0
	o[2] = o2[2]
	rgb[0] = r + 25
	...
	rgb[2] = b + 25
	new a
	for( a = 0; a < 3; a++ ) if(rgb[a] > 255) rgb[a] = 255
	rgb2[0] = r
	rgb2[1] = g
	rgb2[2] = b
	
	ent = create_entity("info_target")
	set_pev( ent2, pev_iuser1, ent )
	temp_entity[ent] = 1
	set_pev( ent, pev_classname, "zn_glow" )
	engfunc( EngFunc_SetModel, ent, xLoadSpriteFlareGlow );
	set_pev( ent, pev_frame, 0 )
	set_rendering(ent, kRenderFxNone, rgb[0], rgb[1], rgb[2], kRenderTransAdd, 220)
	...
	set_pev( ent, pev_scale, 4.4 )
	set_pev( ent, pev_vuser1, rgb2 )
	set_pev( ent, pev_frame, 0 )
	set_pev( ent, pev_team, 6666 )
	
	//zn_emit( cfg_flare_on, ent2, CHAN_WEAPON, 1.0, ATTN_NORM, 0, PITCH_NORM  )
	set_pev( ent, pev_iuser1, ent2 )
	set_pev( ent, pev_fuser1, get_gametime() + float( get_pcvar_num( cvar_flareduration )))
	return HAM_IGNORED
}

public infotarget_think( ent ){
	static id
	if( !is_valid_ent( ent ))
		return HAM_IGNORED;
	...
	switch( pev( ent, pev_team )){
		case 6666:{	
			static Float:e, ent2;
			pev( ent, pev_fuser1, e )
			ent2 = pev( ent, pev_iuser1 )
			...
			pev(ent, pev_vuser1, rgb)
			o[2] += 1.0;
			o[1] -= 4.0
			

			// Lighting
			engfunc(EngFunc_MessageBegin, MSG_PAS, SVC_TEMPENTITY, o, 0)
			//engmessage_begin2( MSG_PAS, SVC_TEMPENTITY, o, 0)
			write_byte(TE_DLIGHT) // TE id
			engfunc(EngFunc_WriteCoord, o[0]) // x
			engfunc(EngFunc_WriteCoord, o[1]) // y
			engfunc(EngFunc_WriteCoord, o[2]) // z
			...
			if( e < get_gametime() + 0.9 )
				write_byte(20) //decay rate
			
			else write_byte(0) //decay rate
			...
			{
				rgb2[a] = rgb[a] + 60
				if( rgb2[a] > 255 ) rgb2[a] = 255
			}
			
			/* lag check
			engmessage_begin2( MSG_BROADCAST, SVC_TEMPENTITY, o, 0)
			write_byte(TE_ELIGHT)
			write_short(ent)
			engfunc(EngFunc_WriteCoord, o[0]) // x
			engfunc(EngFunc_WriteCoord, o[1]) // y
			engfunc(EngFunc_WriteCoord, o[2] + 10.0) // z
			write_coord(cfg_int(cfg_flaresize) + 10) // radius
			write_byte(rgb2[0]) // r
			write_byte(rgb2[1]) // g
			write_byte(rgb2[2]) // b
			write_byte(12) //life
			...
			*/
			...
			// Sparks
			engfunc(EngFunc_MessageBegin, MSG_PAS, SVC_TEMPENTITY, o, 0)
			//engmessage_begin2( MSG_PVS, SVC_TEMPENTITY, o, 0)
		...
		case 7777:
		{
			...
			if( !is_user_connected(id) || !is_user_alive(id) || !g_zombie[ id ] || g_nemesis[ id ] || !g_frozen[id] || g_nodamage[id]) 
			{
				static Float:origin2[3]
				pev( ent, pev_origin, origin2 )
				
				static a
				for( a = 0; a < random_num( 9, 14 ); a++ )
				{
					engfunc(EngFunc_MessageBegin, MSG_PAS, SVC_TEMPENTITY, origin2, 0)
					write_byte(TE_BREAKMODEL) // TE id
					engfunc(EngFunc_WriteCoord,origin2[0]+random_float( -20.0, 20.0 )) // x
					engfunc(EngFunc_WriteCoord,origin2[1]+random_float( -20.0, 20.0 )) // y
					engfunc(EngFunc_WriteCoord,origin2[2]+random_float( 20.0, 60.0 )) // z
					write_coord(16) // size x
					write_coord(16) // size y
					write_coord(16) // size z
					write_coord(random_num(-5, 5)) // velocity x
					write_coord(random_num(-5, 5)) // velocity y
					write_coord(random_num( 5, 10 )) // velocity z
					write_byte(10) // random velocity
					write_short( g_glassSpr ) // model
					write_byte(5) // count
					write_byte(random_num( 20, 35 )) // life
					write_byte(BREAK_GLASS) // flags
					message_end()
				}
				g_frozen[id] = 0
				f_unfreeze_time[id] = 0.0
				// Unfreeze
				g_frozen[id] = false;
				...
				//zn_emit( cfg_frost_break, id,  CHAN_BODY,  1.0, ATTN_NORM, 0, PITCH_NORM )
			...
			if( f_unfreeze_time[id] > get_gametime() ) 
			{
				set_pev( ent, pev_nextthink, get_gametime() + 0.1 )
				return HAM_IGNORED
			}
...
public fw_PlayerPreThink(id)
{
	...
	if (g_nvisionenabled[id] && current_time - g_lastnvg[id] >= 0.05)
	{
		set_user_nvision(id)
		g_lastnvg[id] = current_time
	}
	...
	if(button & (IN_JUMP | IN_DUCK) == (IN_JUMP | IN_DUCK) || button & IN_RELOAD)
	{
			
		if(g_survivor[id] && !(button & IN_RELOAD))
		return 
...
public show_menu_game( id ){
	...
	if( MenuPrincipal[ id ]){
		...
		DestroyMenuPrincipal( id );
	}
	
	MenuPrincipal[ id ] = true;

	new Float: iMenuPos[ 2 ];
	iMenuPos[ 0 ] = 0.02 * 100.0 / 100.0;
	iMenuPos[ 1 ] = 0.42 * 100.0 / 100.0;
	
	set_hudmessage( 146, 222, 14, iMenuPos[ 0 ], iMenuPos[ 1 ], 0, 6.0, 999.0, _, _, 1 );	
	ShowSyncHudMsg( id, xMSGSync[ MENU_TITLE ], "[ TsWBR ]" );
	
	set_hudmessage( 255, 162, 0, iMenuPos[ 0 ], iMenuPos[ 1 ], 0, 6.0, 999.0, _, _, 2 );	
	ShowSyncHudMsg( id, xMSGSync[ MENU_NUMBERS ], "                Menu Principal:^n^n^n1.^n2.^n3.^n4.^n^n5. Zombie-XP^n^n6.^n7.^n8.^n^n9.^n^n0. Sair" );
	
	set_hudmessage( 14, 151, 222, iMenuPos[ 0 ], iMenuPos[ 1 ], 0, 6.0, 999.0, _, _, 3 );	
	ShowSyncHudMsg( id, xMSGSync[ MENU_ITEM1 ], "^n^n^n    Comprar Armas^n    Comprar itens extras^n    Classes de Zombies^n    Destravar^n^n^n^n    Configuracoes^n    Inventario^n    Loja^n^n    Admin Menu" );
	
	show_menu( id, KEYSMENU, " ", _, "Game Menu" );
	set_task( 0.1, "CheckMenuPrincipal", id, _, _, "b" );
	...
	/*
	len += formatex(menu[len], charsmax(menu) - len, "\d%s^n%s^n^n", xPrefix, xMenuLine );
	
	if( get_pcvar_num( cvar_buycustom ))
		len += formatex( menu[ len ], charsmax(menu) - len, "\d1.\r Comprar Armas^n")
		
	else len += formatex(menu[len], charsmax(menu) - len, "\d1. Comprar Armas^n")
	
	if (get_pcvar_num(cvar_extraitems) && g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "\d2.\r Comprar itens extra^n");
		
	else len += formatex(menu[len], charsmax(menu) - len, "\d2. Comprar itens extra^n");
	
	if (get_pcvar_num(cvar_zclasses))
		len += formatex(menu[len], charsmax(menu) - len, "\d3.\r Classes de Zombies^n");
		
	else len += formatex(menu[len], charsmax(menu) - len, "\d3. Classes de Zombies^n");
	
	if (g_isalive[id] && g_zombie[id])
		len += formatex(menu[len], charsmax(menu) - len, "\d4.\r Destravar^n");
	
	else len += formatex(menu[len], charsmax(menu) - len, "\d4. Destravar^n");
	
	len += formatex(menu[len], charsmax(menu) - len, "\d5.\r ZombieXP^n^n");
	
	len += formatex(menu[len], charsmax(menu) - len, "\d6.\r Configuracoes^n^n");
	
	// 6. Join spec

	if (!g_isalive[id] || !get_pcvar_num(cvar_blocksuicide) || (userflags & g_access_flag[ACCESS_ADMIN_MENU]))
		len += formatex(menu[len], charsmax(menu) - len, "\r6.\w Entrar no modo Espectator^n^n");
		
	else len += formatex(menu[len], charsmax(menu) - len, "\d6. Entrar no modo Espectator^n^n")

	
	
	// Loja
	//len += formatex(menu[len], charsmax(menu) - len, "\r7. Loja Live.Play Shop^n")
	//len += formatex(menu[len], charsmax(menu) - len, "\r8. Inventario^n^n")
	
	// 8. Tem Guild
	
	if(has_guild(id))
		len += formatex(menu[len], charsmax(menu) - len, "\r7.\w Menu da Guild^n")


	if (userflags & g_access_flag[ACCESS_ADMIN_MENU])
		len += formatex(menu[len], charsmax(menu) - len, "\r9.\w Admin Menu");
		
	else len += formatex(menu[len], charsmax(menu) - len, "\d9. Admin Menu")
	
	// 0. Exit
	len += formatex(menu[len], charsmax(menu) - len, "^n^n\r0.\w Sair")
	
	*/
	
	//show_menu(id, KEYSMENU, menu, -1, "Game Menu")
	//show_menu(id, KEYSMENU, " ", -1, "Game Menu")
	
}

public CheckMenuPrincipal( id ){
	if( !MenuPrincipal[ id ]){
		if( task_exists( id ))
			remove_task( id );
...
public DestroyMenuPrincipal( id ){
	for( new i = 0; i < 3; i++ ){
...
show_menu_extras(id)
{
	...
	for (item = 0; item < g_extraitem_i; item++)
	{
		...
		switch (item)
		{
			...
			case EXTRA_INFBOMB:
			{
				if (!get_pcvar_num(cvar_extrainfbomb) || xBuyInfectBomb /*g_infbombcounter >= get_pcvar_num(cvar_infbomblimit)*/ ) continue;
				//if (fnGetAliveCTs() <= 3) continue; // Granada de infecção // PAUSADO PRA TESTES
		...
		if (is_user_admin(id))
		{
			formatex(menu, charsmax(menu), "%s \d[\r%d\d %L]", buffer, (ArrayGetCell(g_extraitem_cost, item)*80)/100, id, "AMMO_PACKS2") // 20% de desconto no item...
		}
		else formatex(menu, charsmax(menu), "%s \d[\r%d\d %L]", buffer, ArrayGetCell(g_extraitem_cost, item), id, "AMMO_PACKS2")
...
show_menu_player_list(id)
{
	...
	menu_setprop(menuid, MPROP_BACKNAME, "Voltar")
	menu_setprop(menuid, MPROP_NEXTNAME, "Proximo")
	menu_setprop(menuid, MPROP_EXITNAME, "Sair")
...
public menu_game(id, key)
{
	switch (key)
	{
		...
		case 1: // Extra Items
		{
			...
			{
				new savekey[34]
				get_user_authid(id, savekey, charsmax(savekey))
				
				/*
				if ( !registro_user_liberado(id) ) {
					zp_colored_print(id, "^x04[ZP]^x01 Voce nao tem autorizacao.")
				}
				else show_menu_extras(id)
				*/
		...
		/*
		case 6:{
			client_cmd( id, "say /loja")
		}
		*/
		
		case 6: // guild menu
		{
			//client_cmd( id, "say /inventario")
			/*
			if(has_guild(id))
				client_cmd(id, "guild")
			*/
...
buy_primary_weapon(id, selection)
{
	...
	if(user_has_weapon(id,CSW_G3SG1) || user_has_weapon(id, CSW_M249 ) || user_has_weapon(id, CSW_SG550) || user_has_weapon(id, CSW_AWP) )
	{
		// bug que não da granadas pra quem tem arma especial
		static wname[32]
		static i
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	/* PAUSADO PRA TESTES 
	if ((itemid == EXTRA_INFBOMB && fnGetAliveCTs() <= 3)) // bugfix - 14/09/2012
	{
		zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT")
		return;
	}
	*/
	...
	|| (itemid == EXTRA_INFBOMB && (!get_pcvar_num(cvar_extrainfbomb) || xBuyInfectBomb /* || g_infbombcounter >= get_pcvar_num(cvar_infbomblimit) */ )) // PAUSADO PRA TESTES
	...
	if (!ignorecost)
	{
		if (is_user_admin(id)) //#VIP
		{
			...
			if (g_ammopacks[id] < (ArrayGetCell(g_extraitem_cost, itemid)*80)/100) // 20% de desconto
			{
				zp_colored_print(id, "^x04[ZP]^x01 %L", id, "NOT_ENOUGH_AMMO")
				return;
			}
			...
			// Deduce item cost
			g_ammopacks[id] -= (ArrayGetCell(g_extraitem_cost, itemid)*80)/100	 // 20% de desconto		
		}
		else
		{
			// Check that we have enough ammo packs
	...
	switch (itemid)
	{
		...
		*/
		...
		case EXTRA_ANTIDOTE: // Antidote
		{
			...
			log_player_event(id, "triggered", "Usou_Antidoto", 0)
			log_player_event(id, "triggered", "Comprou_No_Shop", 0)
		}
		...
		{
			/* PAUSADO PRA TESTES*
			if( fnGetAliveCTs() <= 3 ) // 3 ou menos CTs vivos, sem comprar nada noob!
			{
				return; // stop here
			}
			*/
			...
			if (user_has_weapon(id, CSW_HEGRENADE))
			{
				...
				log_player_event(id, "triggered", "Bomba_de_Infectar", 0)
				log_player_event(id, "triggered", "Comprou_No_Shop", 0)
				ExecuteForward(g_fwExtraItemSelected, g_fwDummyResult, id, itemid)
		...
		default:
		{
			...
			else // Custom additions
			{
				...
				if (g_fwDummyResult >= ZP_PLUGIN_HANDLED && !ignorecost)
				{
					if (is_user_admin(id))
					{
						g_ammopacks[id] += (ArrayGetCell(g_extraitem_cost, itemid)*80)/100
					}
					else g_ammopacks[id] += ArrayGetCell(g_extraitem_cost, itemid)
				}
			}
		}
	}
...
public menu_info(id, key)
{
	...
	switch (key)
	{
		case 0: // General
		{
			...
			len += formatex(motd[len], charsmax(motd) - len, "%L ", id, "MOTD_INFO11", "Zombie Plague", VERSION, AUTHOR )
		...
		case 1: // Humans
		{
			...
			len += formatex(motd[len], charsmax(motd) - len, "%L", id, "MOTD_INFO2_C", get_pcvar_float(cvar_humanspd))
		...
		case 3: // Gameplay Modes
		{
			...
			if (get_pcvar_num(cvar_nem))
			{
				...
				len += formatex(motd[len], charsmax(motd) - len, "%L", id, "MOTD_INFO4_D", get_pcvar_float(cvar_nemspd))
			...
			if (get_pcvar_num(cvar_surv))
			{
				...
				len += formatex(motd[len], charsmax(motd) - len, "%L", id, "MOTD_INFO4_K", get_pcvar_float(cvar_survspd))
...
public message_health(msg_id, msg_dest, msg_entity)
{
	...
	if (health % 256 == 0)
		fm_set_user_health2(msg_entity, pev(msg_entity, pev_health) + 1)
...
make_a_zombie(mode, id)
{
	...
	if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_SURVIVOR) && random_num(1, get_pcvar_num(cvar_survchance)) == get_pcvar_num(cvar_surv) && iPlayersnum >= get_pcvar_num(cvar_survminplayers)) || mode == MODE_SURVIVOR)
	{
		...
		zp_colored_print(0, "%L", LANG_PLAYER, "EXTRA_MENSAGEM5", g_playername[forward_id])
	...
	{		
		...
		zp_colored_print(0, "^x04[CSP Gaming ZMXP] ^x03Modo Swarm^x01!")
	...
	else if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_MULTI) && random_num(1, get_pcvar_num(cvar_multichance)) == get_pcvar_num(cvar_multi) && floatround(iPlayersnum*get_pcvar_float(cvar_multiratio), floatround_ceil) >= 2 && floatround(iPlayersnum*get_pcvar_float(cvar_multiratio), floatround_ceil) < iPlayersnum && iPlayersnum >= get_pcvar_num(cvar_multiminplayers)) || mode == MODE_MULTI)
	{
		...
		zp_colored_print(0, "^x04[CSP Gaming ZMXP] ^x03Infeccao Multipla^x01!")
	...
	&& iPlayersnum-(get_pcvar_num(cvar_plaguesurvnum)+get_pcvar_num(cvar_plaguenemnum)+floatround((iPlayersnum-(get_pcvar_num(cvar_plaguenemnum)+get_pcvar_num(cvar_plaguesurvnum)))*get_pcvar_float(cvar_plagueratio), floatround_ceil)) >= 1 && iPlayersnum >= get_pcvar_num(cvar_plagueminplayers)) || mode == MODE_PLAGUE)
	{
		...
		while (iSurvivors < iMaxSurvivors)
		{
			...
			fm_set_user_health2(id, floatround(float(pev(id, pev_health)) * get_pcvar_float(cvar_plaguesurvhpmulti)))
		}
		...
		while (iNemesis < iMaxNemesis)
		{
			...
			fm_set_user_health2(id, floatround(float(pev(id, pev_health)) * get_pcvar_float(cvar_plaguenemhpmulti)))
		}
		...
		zp_colored_print(0, "^x04[CSP Gaming ZMXP] ^x03Modo Plague^x01!")
	...
	else
	{
		...
		new iJogadorPerto, Float:flOrigemMaisPerto = 9999.0
		new Float:flDist
		...
		for (id = 1; id <= g_maxplayers; id++)
		{
			...
			flDist = fm_entity_range(id, forward_id); 
		
			if(flDist < flOrigemMaisPerto)
			{
				iJogadorPerto = id
				flOrigemMaisPerto = flDist
			}
		...
		if (g_nemround)
		{
			...
			zp_colored_print(0, "%L", LANG_PLAYER, "EXTRA_MENSAGEM1", g_playername[forward_id])
		...
		else
		{
			...
			zp_colored_print(0, "%L", LANG_PLAYER, "EXTRA_MENSAGEM2", g_playername[forward_id])
			...
			if(get_playersnum(1) >= 12)
			{
				zombieme(iJogadorPerto, 0, 0, 0, 0) // lol
				zp_colored_print(0, "%L", LANG_PLAYER, "EXTRA_MENSAGEM3", g_playername[iJogadorPerto])
				g_hasleap[iJogadorPerto] = true
			}
		}
	}
...
zombieme(id, infector, nemesis, silentmode, rewards)
{
	...
	if (rewards && infector)
	{
		...
		fm_set_user_health2(infector, pev(infector, pev_health) + get_pcvar_num(cvar_zombiebonushp))
	}
	...
	if (!silentmode)
	{
		if (nemesis)
		{
			...
			log_player_event(id, "triggered", "Virou_O_Nemesis", 0)
			
			// Set health [0 = auto]
			if (get_pcvar_num(cvar_nemhp) == 0)
			{
				if (get_pcvar_num(cvar_nembasehp) == 0)
					fm_set_user_health2(id, ArrayGetCell(g_zclass_hp, 0) * fnGetAlive())
				else
					fm_set_user_health2(id, get_pcvar_num(cvar_nembasehp) * fnGetAlive())
			}
			else
				fm_set_user_health2(id, get_pcvar_num(cvar_nemhp))
		...
		else if (fnGetZombies() == 1)
		{
			...
			fm_set_user_health2(id, floatround(float(ArrayGetCell(g_zclass_hp, g_zombieclass[id])) * get_pcvar_float(cvar_zombiefirsthp)))
			
			// Set gravity, if frozen set the restore gravity value instead
			if (!g_frozen[id]) set_pev(id, pev_gravity, (Float:ArrayGetCell(g_zclass_grav, g_zombieclass[id])-g_gravity_zm[id]))
			else g_frozen_gravity[id] = (Float:ArrayGetCell(g_zclass_grav, g_zombieclass[id])-g_gravity_zm[id])
		...
		else
		{
			...
			// Set health and gravity, unless frozen
			fm_set_user_health(id, ArrayGetCell(g_zclass_hp, g_zombieclass[id]) + g_hp[id])
			...
			{
				zp_colored_print(0, "%L", LANG_PLAYER, "EXTRA_MENSAGEM4", g_playername[id], g_playername[infector])
				log_player_event(infector, "triggered", "Infectou_Alguem", 0)
				log_player_event(id, "triggered", "Foi_Infectado", 0)
			}
		}
	}
	else
...
humanme(id, survivor, silentmode)
{
	...
	if (survivor)
	{
		...
		log_player_event(id, "triggered", "Virou_O_Survivor", 0)
		
		// Set Health [0 = auto]
		if (get_pcvar_num(cvar_survhp) == 0)
		{
			if (get_pcvar_num(cvar_survbasehp) == 0)
				fm_set_user_health2(id, get_pcvar_num(cvar_humanhp) * fnGetAlive())
			else
				fm_set_user_health2(id, get_pcvar_num(cvar_survbasehp) * fnGetAlive())
		}
		else
			fm_set_user_health2(id, get_pcvar_num(cvar_survhp))
	...
	else
	{
		...
		// Set gravity, if frozen set the restore gravity value instead
		if (!g_frozen[id]) set_pev(id, pev_gravity, g_gravity_human[id])
		else g_frozen_gravity[id] = g_gravity_human[id]
	...
	if (get_pcvar_num(cvar_nvggive) && g_survivor[id])
	{
		g_nvision[id] = true
...
load_customization_from_files()
{
		...
		switch (section)
		{
			...
			case SECTION_WEAPON_MODELS:
			{
				if (equal(key, "V_KNIFE HUMAN"))
					...
					copy(w_model_grenade_flare, charsmax(w_model_grenade_flare), value)
			...
			case SECTION_GRENADE_SPRITES:{
				...
				// INFECT:
				if( equal( key, "INFECT_TRAIL"))
					copy( xLoadSpriteInfectTrail, charsmax( xLoadSpriteInfectTrail ), value );
				
				else if( equal( key, "INFECT_GIB"))
					copy(xLoadSpriteInfectGib, charsmax( xLoadSpriteInfectGib ), value );
				
				else if( equal( key, "INFECT_GIB2"))
					copy(xLoadSpriteInfectGib2, charsmax( xLoadSpriteInfectGib2 ), value );
				
				else if( equal( key, "INFECT_EXP"))
					copy( xLoadSpriteInfectExp, charsmax( xLoadSpriteInfectExp ), value );
				
				else if( equal( key, "INFECT_SMOKE"))
					copy( xLoadSpriteInfectSmoke, charsmax( xLoadSpriteInfectSmoke ), value );
				
				
				// FIRE:
				else if( equal( key, "FIRE_TRAIL"))
					copy( xLoadSpriteFireTrail, charsmax( xLoadSpriteFireTrail ), value );
				
				else if( equal( key, "FIRE_GIB"))
					copy( xLoadSpriteFireGib, charsmax( xLoadSpriteFireGib ), value );
				
				else if( equal( key, "FIRE_GIB2"))
					copy( xLoadSpriteFireGib2, charsmax( xLoadSpriteFireGib2 ), value );
				
				else if( equal( key, "FIRE_EXP"))
					copy( xLoadSpriteFireExp, charsmax( xLoadSpriteFireExp ), value );
				
				else if( equal( key, "FIRE_SMOKE"))
					copy( xLoadSpriteFireSmoke, charsmax( xLoadSpriteFireSmoke ), value );
					
				
				// FROST:				
				else if( equal( key, "FROST_TRAIL"))
					copy( xLoadSpriteFrostTrail, charsmax( xLoadSpriteFrostTrail ), value );
				
				else if( equal( key, "FROST_GIB"))
					copy( xLoadSpriteFrostGib, charsmax( xLoadSpriteFrostGib ), value );
				
				else if( equal( key, "FROST_GIB2"))
					copy( xLoadSpriteFrostGib2, charsmax( xLoadSpriteFrostGib2 ), value );
				
				else if( equal( key, "FROST_EXP"))
					copy( xLoadSpriteFrostExp, charsmax( xLoadSpriteFrostExp ), value );
				
				else if( equal( key, "FROST_SMOKE"))
					copy( xLoadSpriteFrostSmoke, charsmax( xLoadSpriteFrostSmoke ), value );
				
				
				// FLARE:
				else if( equal( key, "FLARE_TRAIL"))
					copy( xLoadSpriteFlareTrail, charsmax(xLoadSpriteFlareTrail), value)
				
				else if( equal( key, "FLARE_GLOW"))
					copy( xLoadSpriteFlareGlow, charsmax(xLoadSpriteFlareGlow), value)
					
				else if (equal(key, "RING"))
					copy(sprite_grenade_ring, charsmax(sprite_grenade_ring), value)
				
				else if (equal(key, "FLAME"))
			...
			case SECTION_INFECT: {
				if( equal( key, "INFECT_EFFECT"))
					copy( xLoadSpriteInfectEffect, charsmax( xLoadSpriteInfectEffect ), value );
			}
	...
	if (file_exists(path))
	{
		// Open extra item s file for reading
		file = fopen(path, "rt")
...
save_customization()
{
	...
	// Build extra item s file path
	get_configsdir(path, charsmax(path))
	format(path, charsmax(path), "%s/%s", path, ZP_EXTRAITEMS_FILE)
	
	// Open extra ite ms file for appending data
	file = fopen(path, "at")
	size = ArraySize(g_extraitem_name)
	
	// Add any new extra it ems data at the end if needed
...
// Bots automatically buy extra i tems
...
{
	// Show mod info
	zp_colored_print(0, "^x04[CSP Gaming]^x01 Adicione nos favoritos e chame seus amigos para jogar!")
	zp_colored_print(0, "^x01Visite nosso forum:^x03 www.cspgaming.com.br/forum")
	//zp_colored_print(0, "^x04 BRGaMes!CS ZombieXP^x01 - O servidor de ZM mais jogado do^x03 BRASIL!")
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		if (get_pcvar_num(cvar_keephealthondisconnect) && g_nemesis[leaving_player])
			fm_set_user_health2(id, pev(leaving_player, pev_health))
	}
...
/*
stock CreateInfectSmoke( Float: iOrigin[ 3 ]){
	static ent
	...
	engfunc( EngFunc_SetModel, ent, xLoadSpriteInfectSmoke );
	entity_set_string( ent, EV_SZ_classname, "fsmoke" );
	
	set_pev( ent, pev_rendermode, kRenderTransAdd);
	set_pev( ent, pev_renderamt, 255.0);
	//set_pev( ent, pev_light_level, 180);
	set_pev( ent, pev_scale, random_float(1.0, 1.5));
	...
	//set_pev( ent, pev_iuser4, SMOKE_INFECT );
	set_pev( ent, pev_iuser4, 5612 )
	
	dllfunc( DLLFunc_Spawn, ent );
	drop_to_floor( ent );
	
	set_task( 15.0, "RemoveInfectSmoke", ent );
...
public RemoveInfectSmoke( entity ){
	if( pev_valid( entity )){
		remove_task( entity );
...
stock zn_create_infect_smoke( ent, count ){
...
infection_explode( ent ){
	...
	if( pev( ent, pev_team ) == 8 ){
		static Float:u
		pev( ent, pev_fuser2, u )
		...
		if( u < get_gametime()){
			...
			engfunc( EngFunc_WriteCoord, iOrigin[ 2 ] + 4.0 );
		...
		if( !CreateInfectBlast( ent, originF ))
			return HAM_SUPERCEDE;
		
		set_pev( ent, pev_team, 8 );
		static Float:t, g
		g = random_num( 240, 250 )
		t = get_gametime() + (float(g) * 0.1 )
		set_pev( ent, pev_fuser2, t )
		zn_create_infect_smoke( ent, 5 );
...
public zn_smoke_think( ent ){
	if( !is_valid_ent( ent ))
		return HAM_IGNORED;
	
	switch( pev( ent, pev_iuser4 )){
		case SMOKE_INFECT: {
			static Float:s, Float:s2, l
			l = pev( ent, pev_iuser3 );
			...
			static Float:r
			pev( ent, pev_fuser1, r )
			pev( ent, pev_fuser2, s )

			if( r < get_gametime()){
				pev( ent, pev_scale, s2)
				...
				if( s2 < s ){
					...
					entity_set_size( ent, mins, maxs );
					set_pev( ent, pev_scale, s2 );
				}
			}
			
			static Float:v[3], Float:o[3]
			pev( ent, pev_origin, v )
			pev( ent, pev_vuser2, o )
			
			if( l < 40 ){
				pev( ent, pev_renderamt, r )
				s = float(l) / 40
				
				if( s < 0.0 || s > 255.0 )
					s = 0.0;
				
				set_pev( ent, pev_renderamt, 80.0 * s )
			}
				...
				set_pev( ent, pev_renderamt, r )
			}
			
			
			v[0] += o[0]; v[1] += o[1]; v[2] += o[2];
			o[0] /= 1.01; o[1] /= 1.01 ; o[2] /= 1.01
			
			set_pev( ent, pev_vuser2, o )
			set_pev( ent, pev_origin, v )
			set_pev( ent, pev_iuser3, l )
		}
		...
		case SMOKE_FROST:
		{
			...
			//client_print( 0, print_chat, "Think 1");
			...
			if(r < get_gametime())
			{
				...
				if( s2 < s )
				{
					...
					entity_set_size(ent, mins, maxs )
					set_pev( ent, pev_scale, s2 )
					//client_print( 0, print_chat, "Think 2");
				}
			}
			...
			//client_print( 0, print_chat, "Think 3");
		}
	}
	return HAM_IGNORED
}

new Float:smokedir[5] = { 30.0, 112.0, 187.0, 235.0, 299.0 }
stock zn_infectsmoke_think( ent ){
	static Float:u
	pev( ent, pev_fuser4, u )
	
	if( u > get_gametime())
		return;
		
	static i, s
	i = pev(ent, pev_iuser4)
	s = pev(ent, pev_iuser3)
	
	if(i){
		static Float:originF[3], Float:o[3], Float:o2[3], Float:d[3]
		pev( ent, pev_origin, originF )
		if(s > 4) s = 0
		d[0] = random_float( -88.0, -40.0 )
		d[1] = smokedir[s] + random_float( -10.0, 10.0 )
		VelocityByAngle( d, random_float( 1.5, 2.0 ), o2)
		o2[2] /= 2.0;
		o[0] = originF[0] + random_float( -10.0, 10.0 )
		o[1] = originF[1] + random_float( -10.0, 10.0 )
		o[2] = originF[2] + random_float( 0.0, 20.0 )
		zn_create_infect_smoke2( pev( ent, pev_owner ), o, o2, random_num( 240, 250 ), random_float( 1.5, 1.9 ))
		i--
		s++
...
stock zn_create_infect_smoke2( owner, Float:o[3], Float:dir[3], life, Float:size ){
	static ent
	ent = create_entity("env_sprite")
	if( !is_valid_ent( ent ))
		return;
	
	set_pev( ent, pev_classname, "fsmoke" )
	entity_set_model(ent, xLoadSpriteInfectSmoke)
	set_rendering(ent, kRenderFxNone, 255, 255, 255, kRenderTransAdd, 0)
	set_pev(ent, pev_animtime, get_gametime() )
	set_pev(ent, pev_framerate, 10.0 )
	set_pev(ent, pev_spawnflags, SF_SPRITE_STARTON)
	dllfunc(DLLFunc_Spawn, ent)
	...
	maxs[1] = 48.0 * 0.1;
	maxs[2] = 48.0 * 0.1;
	set_pev( ent, pev_iuser3, life )
	set_pev( ent, pev_fuser2, size )
	set_pev( ent, pev_vuser2, dir )
	set_pev( ent, pev_iuser4, SMOKE_INFECT )
	set_pev( ent, pev_owner, owner )
	
	entity_set_size(ent, mins, maxs )
	set_pev( ent, pev_scale, random_float( 0.2, 0.4) )
	set_pev( ent, pev_nextthink, get_gametime() + 0.1 )
	set_pev( ent, pev_vuser1, o )
}
...
public infection_explode( ent ){
	...
	CreateInfectBlast( originF );
	CreateInfectSmoke( originF );
...
public fsmoke_touch( ent, id ){
	if( !is_valid_ent( ent ) || !is_user_connected( id ) || !is_user_alive( id ))
		return HAM_IGNORED;
	
	/*
	static iClassname[32];
	pev( ent, pev_classname, iClassname, sizeof( iClassname ))
	
	if( equal( iClassname, "fsmoke" )){	
	}
	*/
	
	new iSmoke = pev( ent, pev_iuser4 );
	switch( iSmoke ){
		case SMOKE_INFECT: {			
			if( inf_touch[ id ] > get_gametime())
				return HAM_IGNORED;
			...
			if( g_zombie[ id ]){
				if( inf_time[ id ] < get_gametime()){
					static maxhp, hp
					maxhp = ArrayGetCell( g_zclass_hp, g_zombieclass[ id ]);
					
					if( g_firstzombie[ id ])
						maxhp = floatround( float( maxhp ) * get_pcvar_float( cvar_zombiefirsthp ))
					
					hp = get_user_health( id );
					
					if( hp < maxhp ){
						hp += random_num( 150, 250 );
						
						if( hp > maxhp )
							fm_set_user_health2( id, maxhp );
							
						else fm_set_user_health2( id, hp );
					}
					
					inf_time[ id ] = get_gametime() + random_float( 0.2, 0.8 );
				}
				
				//set_pev( id, pev_maxspeed, g_zombie_spd[ id ] * 4.0 );
			}
			
			else {
				if( inf_dmgtime[ id ] < get_gametime()){
					static armor, hp
					armor = pev( id, pev_armorvalue )
					
					if( armor > 0 ){
						armor -= random_num( 15, 25 );
						...
						else set_pev( id, pev_armorvalue, float( armor ));
						
						inf_dmgtime[ id ] = get_gametime() + random_float( 0.9, 1.5 );
					}
						...
						if( hp <= 0 ){
							...
							if( is_user_connected(i)){
								for( u = 1; u < 33; u++ ){
									if( !is_user_connected( u ) || !is_user_alive( u ) || g_zombie[ u ])
										continue;
								...
								if( h > 1 )
									zombieme( id, pev( ent, pev_owner ), 0, 1, 1);
									
								else ExecuteHamB(Ham_Killed, id, i, 0);
						...
						inf_dmgtime[ id ] = get_gametime() + random_float( 0.1, 0.3 );
					}
				}
			}
			...
			write_short(UNIT_SECOND / 8) // hold time
			write_short(0x0002) // fade type
			write_byte( 99 ) // red
			write_byte( 198 ) // green
			write_byte( 0 ) // blue
			write_byte( 255 ) // alpha
			message_end()
				
			inf_touch[ id ] = get_gametime() + 0.1
		}
		
		case SMOKE_FROST: {
			if( !g_zombie[ id ] || g_nemesis[ id ] || g_frozen[id] || g_nodamage[id] || fsmoke_touching[id] > get_gametime())
				return HAM_IGNORED
			...
			fsmoke_touching[id] = get_gametime() + 0.1
			...
			//set_pev( id, pev_maxspeed, g_zombie_spd[id] * 0.8 )
		}
	}
...
public fire_explode(ent){
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0){
		...
		if (!is_user_valid_alive(victim) || !g_zombie[victim] || g_nodamage[victim] || g_flamejante[victim])
			continue;
		...
		////
		//ExecuteHam( Ham_TakeDamage, victim, 0, victim, 10.0, DMG_BURN)
...
frost_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		if (!is_user_valid_alive(victim) || !g_zombie[victim] || g_frozen[victim] || g_nodamage[victim] || g_congelante[victim])
			continue;
		...
		log_player_event(victim, "triggered", "Foi_Congelado", 0)
...
frost_explode(ent){
	...
	if( pev( ent, pev_team ) == 8 ){
		...
		if( u < get_gametime() )
		{
			static Float:v[3]
			pev(ent, pev_origin, v)
			engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, v, 0)
			write_byte(TE_DLIGHT);
			engfunc(EngFunc_WriteCoord, v[0]) // x
			engfunc(EngFunc_WriteCoord, v[1]) // y
			engfunc(EngFunc_WriteCoord, v[2] + 4.0) // z
			write_byte(20); // radius
			write_byte( 3 ) // red
			write_byte( 90 ) // green
			write_byte( 252 ) // blue
		...
		if( !CreateFrostBlast( ent, originF ))
			return HAM_SUPERCEDE;
...
stock StartFrostSmokeThink( ent ){
	static Float:u
	pev( ent, pev_fuser4, u )
	...
	if(i){
		...
		o[1] = originF[1] + random_float( -10.0, 10.0 )
		o[2] = originF[2] + random_float( 0.0, 20.0 )
		zn_create_frost_smoke2( pev( ent, pev_owner ), o, o2, random_num( 240, 250 ), random_float( 1.5, 1.9 ))
		i--
		s++
		set_pev(ent, pev_iuser4, i)
		set_pev(ent, pev_iuser3, i)
...
stock zn_create_frost_smoke2( owner, Float:o[3], Float:dir[3], life, Float:size ){
	static ent
	ent = create_entity("env_sprite")
	if( !is_valid_ent( ent ))
	return;
	...
	set_pev( ent, pev_classname, "fsmoke" )
	entity_set_model(ent, xLoadSpriteFrostSmoke)
	set_rendering(ent, kRenderFxNone, 255, 255, 255, kRenderTransAdd, 0)
	set_pev(ent, pev_animtime, get_gametime() )
	set_pev(ent, pev_framerate, 10.0 )
	set_pev(ent, pev_spawnflags, SF_SPRITE_STARTON)
	...
	set_pev( ent, pev_fuser2, size )
	set_pev( ent, pev_vuser2, dir )
	set_pev( ent, pev_iuser4, SMOKE_FROST )
	set_pev( ent, pev_owner, owner )
...
stock CreateFrostSmoke( ent, count ){
	if( !is_valid_ent(ent) ) return
	set_pev( ent, pev_iuser4, count )
}
...
replace_weapon_models(id, weaponid)
{
	switch (weaponid)
	{
		...
		{
			/*
			new iColor = random( MAX_COLORS );
			while( xColorOwner[ iColor ]){
				iColor = random( MAX_COLORS );
			}
	
			xPlayerFlareColor[ id ] = iColor;
			xColorOwner[ iColor ] = id;
			*/
...
public ShowHUD( taskid ){
	...
	static iClass[ 32 ], iHudColor[ 3 ];
	static Float: fVelocity[ 3 ], iSpeed
	static Float: fGravity, iGravity
	static iAP, iXP, iStamina;
	
	pev( id, pev_velocity, fVelocity );
	iSpeed = floatround( vector_length( fVelocity ));

	pev( id, pev_gravity, fGravity );
	iGravity = floatround( fGravity * 100.0 );
	
	iAP = g_ammopacks[ ID_SHOWHUD ];
	iXP = zmxp_get_user_xp( ID_SHOWHUD );
	iStamina = zp_get_user_stamina( ID_SHOWHUD );
	...
	if( g_zombie[ id ]){		
		if( g_nemesis[ id ]){
			...
			formatex( iClass, charsmax( iClass ), "Nemesis");
		}
			...
			formatex( iClass, charsmax( iClass ), "Zombie: %s", g_zombie_classname[ id ]);
		}
	}
	...
	// HUMANOS:
		...
		if( g_survivor[ id ])
			formatex( iClass, charsmax( iClass ), "Survivor");
			
		else formatex( iClass, charsmax( iClass ), "Humano");
	}
	
	if( id != ID_SHOWHUD ){
		set_hudmessage(255, 255, 255, HUD_SPECT_X, HUD_SPECT_Y, 0, 6.0, 2.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L %s^nHP: %d - %L %s - %L %d", ID_SHOWHUD, "SPECTATING", g_playername[id], pev(id, pev_health), ID_SHOWHUD, "CLASS_CLASS", iClass, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[id])
	}
	
	else {
		set_hudmessage( iHudColor[ 0 ], iHudColor[ 1 ], iHudColor[ 2 ], HUD_STATS_X, HUD_STATS_Y, 0, 6.0, 2.1, 0.0, 0.0, -1)
		ShowSyncHudMsg( ID_SHOWHUD, g_MsgSync2, "[ HP: %d | %s | AP: %d | XP: %i ]^n[ Gravity: %d%% | Speed: %d | Stamina: %i%% ]", pev( ID_SHOWHUD, pev_health ), iClass, iAP, iXP, iGravity, iSpeed, iStamina );
	}
}
	...
	for (id = 1; id <= g_maxplayers; id++)
	{
		...
		if (g_isalive[id] && !g_zombie[id] && !g_survivor[id] && fnGetHumans() == 1)
		{
			if (!g_lasthuman[id])
			{
				...
				// Reward extra hp
				fm_set_user_health2(id, pev(id, pev_health) + get_pcvar_num(cvar_humanlasthp))
			}
			g_lasthuman[id] = true
		}
		else
	...
	else
	{
		if (g_zombie[id])
		{
			if (g_nemesis[id])
				...
				set_pev(id, pev_maxspeed, g_zombie_spd[id] + g_velocidade[id])
		}
		else
		{
			if (g_survivor[id])
				...
				set_pev(id, pev_maxspeed, get_pcvar_float(cvar_humanspd) + g_velocidade[id])
		}
	}
}
...
public native_set_user_frozen(id, set)
{
	...
	if(set) /* Set = 1, froze player */
	{
		if(is_user_valid_alive(id) && !g_frozen[id])
		{
			...
			log_player_event(id, "triggered", "Foi_Congelado", 0)
	...
	else /* Set = 0, unfroze player */
	{
		...
		if (g_zombie[id])
		{
			if (g_nemesis[id])
				...
				set_pev(id, pev_gravity, (Float:ArrayGetCell(g_zclass_grav, g_zombieclass[id])-g_gravity_zm[id]))
		}
		else
		{
			if (g_survivor[id])
...
// Native: zp_user_has_painshockfree
public native_user_has_painshockfree(id)
{
	return g_painshockfree[id]
}

// Native: zp_set_user_painshockfree
public native_set_user_painshockfree(id, set)
{

	g_painshockfree[id] = bool:set

}

// Native: zp_give_user_madness
public native_give_user_madness(id)
{
	g_hasmadness[id] = true
	g_painshockfree[id] = true
	set_task(0.1, "zombie_aura", id + TASK_AURA, _, _, "b")
	set_task(4.0, "madness_over", id+TASK_BLOOD)
...
// Native: zp_set_user_leap     - APENAS PARA ZOMBIES!
public native_set_user_leap(id, set)
{
	g_hasleap[id] = bool:set
}

// Native: zp_user_has_leap
public native_user_has_leap(id)
{
	return g_hasleap[id]
}
...
// Native: zp_set_zombie_congelante
public native_set_zombie_congelante(id, set)
{

	g_congelante[id] = bool:set

}

// Native: zp_set_zombie_flamejante
public native_set_zombie_flamejante(id, set)
{

	g_flamejante[id] = bool:set
...
public native_register_extra_item(const name[], cost, team)
{
	...
	// Override extra item s data with our customizations
...
public set_user_nvision( id ){
	...
	if( inf_touch[ id ] > get_gametime()){
	...
	else if( z_burning[ id ] > 0 ){
		write_byte( 255 ) // red
		write_byte( 198 ) // green
		write_byte( 64 ) // blue
	}
	
	else if( g_frozen[ id ] || fsmoke_touching[ id ] > get_gametime()){
		write_byte( 3 ) // red
		write_byte( 90 ) // green
		write_byte( 252 ) // blue
	}
		...
		if (g_nemesis[id] || (g_zombie[id] && g_nodamage[id]) || (!is_user_alive(id) && g_nemround)){
		...
		else if (!g_zombie[id] || !is_user_alive(id)){
...
infection_effects(id){
	...
	/** EFEITO INFECÇÃO ZOMBIE, GOSMA VERDE E GLOW VERDE POR 5 SEGUNDOS *
	if( !g_nemesis[id]){
		fm_set_rendering( id, kRenderFxGlowShell, 180, 255, 0, kRenderNormal, 16 );
		set_task( 5.0, "RemoveGlowInfect", id );
		
		nade_effect1( iOrigin, xSpriteInfectGib, random_num(3,6), random_num(3,6), 6, 20, 25 )
		nade_effect1( iOrigin, xSpriteInfectGib, random_num(10,15), random_num(3,6),4, 20, 25 )
		nade_effect1( iOrigin, xSpriteInfectGib, random_num(3,6), random_num(3,6),3, 20, 25 )
		
		//UTIL_DLight( iOrigin, 80, FROST_COLORS, 100, 40 );
		//UTIL_DLight( Float:vOrigin[ 3 ], iRadius, color[3], iLife, iDecay )
		...
		//nade_effect4( iOrigin, mdl_index[0], 80, random_num(1,3), random_num(20,30), random_num(20,30));
		//nade_effect4( iOrigin, mdl_index[0], 50, random_num(1,3), random_num(20,30), random_num(20,30));
	}
	*/
...
new zplus_ef_count
new Float:zplus_ef_time

zpplus_infect_effect( id ){
	if( zplus_ef_time < get_gametime())
		zplus_ef_count = 0;
	...
	pev( id, pev_origin, o )
	zplus_ef_count++
	zplus_ef_time = get_gametime() + 2.0
	...
	engfunc(EngFunc_WriteCoord, o[1]) // y
	engfunc(EngFunc_WriteCoord, o[2]) // z
	write_short(xSpriteInfectEffect)
	write_byte(20)
	write_byte(40)
	...
	//ze_fadeout[id] = 0.02
	//ze_life[id] = 50
	//ze_fadeout_color[id][0] = cfg_int(cfg_eff2, 0)
	//ze_fadeout_color[id][1] = cfg_int(cfg_eff2, 1)
	//ze_fadeout_color[id][2] = cfg_int(cfg_eff2, 2)
		
	if( zplus_ef_count < 6 ){
		//fm_set_user_rendering( ent, kRenderFxGlowShell, 255, 32, 0, kRenderNormal, 16 );
		zn_create_glow( o, 255, 32, 0, 1.0, 3, 100 )
			
		static Float:d[3], Float:o3[3], a
		for( a = 0; a < random_num( 5, 8 ); a++ ){
			d[0] = random_float( -30.0, 0.0 )
			d[1] = random_float( 0.0, 360.0 )
			VelocityByAngle( d, random_float( 4.0, 13.0 ), o3) 
				
			engfunc( EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, o, 0)
			write_byte(TE_PROJECTILE)
			engfunc(EngFunc_WriteCoord, o[0] + random_float( -60.0, 60.0 )) // x
			engfunc(EngFunc_WriteCoord, o[1] + random_float( -60.0, 60.0 )) // y
			engfunc(EngFunc_WriteCoord, o[2] + random_float( 4.0, 60.0 )) // z
			engfunc(EngFunc_WriteCoord, o3[0]) // x
			engfunc(EngFunc_WriteCoord, o3[1]) // y
			engfunc(EngFunc_WriteCoord, o3[2]) // z
			write_short( xSpriteInfectGib2 )
			write_byte(random_num( 6, 15 ))
			write_byte(id)
			message_end()
		}
		...
		for( a = 0; a < 12; a++ ){
			...
			engfunc(EngFunc_WriteCoord, o[0] + random_float( -80.0, 80.0 )) // x
			engfunc(EngFunc_WriteCoord, o[1] + random_float( -80.0, 80.0 )) // y
			engfunc(EngFunc_WriteCoord, o[2] + random_float( -20.0, 80.0 )) // z
			message_end();
		}
		...
		engfunc(EngFunc_WriteCoord, o[0]) // x
		engfunc(EngFunc_WriteCoord, o[1]) // y
		engfunc(EngFunc_WriteCoord, o[2] + 4.0) // z
		engfunc(EngFunc_WriteCoord, o[0] + 5.0) // x
		engfunc(EngFunc_WriteCoord, o[1] + 5.0) // y
		engfunc(EngFunc_WriteCoord, o[2] + 100.0) // z
		write_short( xSpriteInfectGib )
		write_byte(random_num(3,4)) // count
		write_byte(24) // life
		write_byte(20) // scale
		write_byte(20) // velocity randomness
		write_byte(15) // velocity randomness
		message_end()
		...
		engfunc(EngFunc_WriteCoord, o[2] + 100.0) // z
		write_short( xSpriteInfectGib )
		write_byte(random_num(4,6)) // count
		write_byte(24) // life
		write_byte(15) // scale
		write_byte(20) // velocity randomness
		write_byte(15) // velocity randomness
		...
		engfunc(EngFunc_WriteCoord, o[2] + 100.0) // z
		write_short( xSpriteInfectGib )
		write_byte(random_num(5,9)) // count
		write_byte(24) // life
		write_byte(8) // scale
		write_byte(20) // velocity randomness
		write_byte(15) // velocity randomness
...
stock zn_create_glow( Float:o[3], r, g, b, Float:scale, life, diespeed ){
	new ent, rgb[3]
	rgb[0] = r + 25
	rgb[1] = g + 25
	rgb[2] = b + 25
	...
	ent = create_entity("info_target")
	set_pev( ent, pev_classname, "zn_glow2" )
	engfunc( EngFunc_SetModel, ent, xLoadSpriteFlareGlow );
	set_rendering(ent, kRenderFxNone, rgb[0], rgb[1], rgb[2], kRenderGlow, 255)
	set_pev(ent, pev_animtime, get_gametime() + 0.1 )
	set_pev(ent, pev_framerate, 0.0 )
	dllfunc(DLLFunc_Spawn, ent)
	set_pev(ent, pev_solid, SOLID_NOT)
	...
	set_pev( ent, pev_iuser1, life )
	set_pev( ent, pev_iuser2, 255 )
	set_pev( ent, pev_iuser3, diespeed )
	set_pev( ent, pev_team, 2550 )
	set_pev( ent, pev_nextthink, get_gametime() + 0.1 )
	set_pev( ent, pev_frame, 1 )
}

public glow_effectz( ent ){
	if( !is_valid_ent( ent ))
		return HAM_IGNORED;
		
	if(pev( ent, pev_team ) != 2550)
		return HAM_IGNORED;
	
	static l, r
	l = pev( ent, pev_iuser1 )
	if( l < 0 ){
		r = pev( ent, pev_iuser2 ) - pev( ent, pev_iuser3 )
		if( r < 0 ){
			remove_entity(ent)
			return HAM_SUPERCEDE
		}
		
		set_pev( ent, pev_renderamt, float(r))
		set_pev( ent, pev_iuser2, r )
	}
	
	else l--
	
	set_pev( ent, pev_iuser1, l )
	set_pev( ent, pev_nextthink, get_gametime() + 0.1 )
	set_pev( ent, pev_frame, 1 )
...
public zombie_aura(taskid)
{
	...
	if (!g_nemesis[ID_AURA] && !g_hasmadness[ID_AURA])
	{
		// Task not needed anymore
		remove_task(taskid);
		return;
...
public burning_flame(taskid)
{
	...
	z_burning[ ID_BURN ] = true;
	
	// Take damage from the fire
	if (health - floatround(get_pcvar_float(cvar_firedamage), floatround_ceil) > 0)
		fm_set_user_health2(ID_BURN, health - floatround(get_pcvar_float(cvar_firedamage), floatround_ceil))
...
stock CreateInfectBlast( entity, const Float:originF[3]){
	// Smallest ring Extra
	engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
	...
	engfunc(EngFunc_WriteCoord, originF[2]+135.0) // z axis
	write_short(g_exploSpr) // id_list 7
	...
	engfunc(EngFunc_WriteCoord, originF[0]) // x axis
	engfunc(EngFunc_WriteCoord, originF[1]) // y axis
	engfunc(EngFunc_WriteCoord, originF[2]+175.0) // z axis
	write_short(g_exploSpr) // id_list7
	write_byte(0) // startframe
	write_byte(0) // framerate
	write_byte(4) // life
	write_byte(60) // width
	...
	engfunc(EngFunc_WriteCoord, originF[0]) // x axis
	engfunc(EngFunc_WriteCoord, originF[1]) // y axis
	engfunc(EngFunc_WriteCoord, originF[2]+270.0) // z axis
	write_short(g_exploSpr) // id_list 7
	write_byte(0) // startframe
	write_byte(0) // framerate
	write_byte(4) // life
	...
	engfunc(EngFunc_WriteCoord, originF[0]) // x axis
	engfunc(EngFunc_WriteCoord, originF[1]) // y axis
	engfunc(EngFunc_WriteCoord, originF[2]+355.0) // z axis
	write_short(g_exploSpr) // id_list 7
	write_byte(0) // startframe
	write_byte(0) // framerate
	write_byte(4) // life
	...
	engfunc(EngFunc_WriteCoord, originF[0]) // x
	engfunc(EngFunc_WriteCoord, originF[1]) // y
	engfunc(EngFunc_WriteCoord, originF[2] + 30.0) // z
	write_short( xSpriteInfectExp );
	write_byte(20)
	write_byte(20)
	write_byte(TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND)
	message_end()
	...
	for( i = 0; i < 6; i++ ){
		...
		engfunc(EngFunc_WriteCoord, originF[1] + random_float( -50.0, 50.0 )) // y
		engfunc(EngFunc_WriteCoord, originF[2] + random_float( 5.0, 50.0 )) // z
		message_end();
	}
	...
	engfunc(EngFunc_WriteCoord, originF[1]) // y
	engfunc(EngFunc_WriteCoord, originF[2] + 4.0) // z
	engfunc(EngFunc_WriteCoord, originF[0] + 5.0) // x
	engfunc(EngFunc_WriteCoord, originF[1] + 5.0) // y
	engfunc(EngFunc_WriteCoord, originF[2] + 100.0) // z
	write_short( xSpriteInfectGib )
	write_byte(random_num(4,6)) // count
	write_byte(24) // life
	...
	engfunc(EngFunc_WriteCoord, originF[2] + 100.0) // z
	write_short( xSpriteInfectGib )
	write_byte(random_num(9,13)) // count
	write_byte(24) // life
	write_byte(8) // scale
	write_byte(20) // velocity randomness
	write_byte(15) // velocity randomness
	...
	// GIBS FLUTUANTES:
	static Float:d[3], Float:o[ 3 ], Float:o2[3]
	for( i = 0; i < random_num( 20, 35 ); i++ ){
		d[0] = random_float( -30.0, 0.0 )
		d[1] = random_float( 0.0, 360.0 )
		VelocityByAngle( d, random_float( 4.0, 13.0 ), o) 
		
		engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
		write_byte(TE_PROJECTILE)
		engfunc(EngFunc_WriteCoord, originF[0] + random_float( -60.0, 60.0 )) // x
		engfunc(EngFunc_WriteCoord, originF[1] + random_float( -60.0, 60.0 )) // y
		engfunc(EngFunc_WriteCoord, originF[2] + random_float( 4.0, 60.0 )) // z
		engfunc(EngFunc_WriteCoord, o[0]) // x
		engfunc(EngFunc_WriteCoord, o[1]) // y
		engfunc(EngFunc_WriteCoord, o[2]) // z
		write_short( xSpriteInfectGib2 )
		write_byte( 50 ) // tempo que os gibs irao flutuar
		write_byte(0)
		message_end()
	}
	...
	for( i = 1; i < 33; i++ ){
		...
		pev( i, pev_origin, o2 )
		d2 = get_distance_f( originF, o2 );
		if( d2 < 250.0 ){
			if( fnGetHumans() == 1 ){
				ExecuteHamB(Ham_Killed, i, attacker, 0)
				continue;
			}
			
			ArrayGetString(grenade_infect_player, random_num(0, ArraySize(grenade_infect_player) - 1), sound, charsmax(sound))
			emit_sound( i, CHAN_VOICE, sound, 1.0, ATTN_NORM, 0, PITCH_NORM)
			
			zombieme( i, attacker, 0, 1, 1)
		}
	}

	/*
	// LUZ ACESA POR 15 SEGUNDOS:
	engfunc(EngFunc_MessageBegin, MSG_PAS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_DLIGHT) // TE id
	engfunc(EngFunc_WriteCoord, originF[ 0 ]) // x
	engfunc(EngFunc_WriteCoord, originF[ 1 ]) // y
	...
	write_byte( 250 ) // green
	write_byte( 5 ); // BLUE
	write_byte( 150 ) //life - 21 (EX: 10 Life =  1 segundo ) / calcular - 10x15
...
stock VelocityByAngle( Float:a[3], Float:b, Float:c[ 3 ]){
	static Float:d
...
stock CreateFireBlast(const Float: originF[3]){
	// Smallest ring Extra
	engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_BEAMCYLINDER) // TE id
	engfunc(EngFunc_WriteCoord, originF[0]) // x
	...
	engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_EXPLOSION)
	engfunc(EngFunc_WriteCoord, originF[0] + 70.0) // x
	engfunc(EngFunc_WriteCoord, originF[1] + 5.0) // y
	engfunc(EngFunc_WriteCoord, originF[2] + -5.0) // z
	write_short( xSpriteFireExp );
	write_byte(random_num( 25, 40 ))
	write_byte(random_num( 8, 14 ))
	write_byte(TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND)
	message_end()
	...
	engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_EXPLOSION)
	engfunc(EngFunc_WriteCoord, originF[0] - 60.0) // x
	engfunc(EngFunc_WriteCoord, originF[1] - 5.0) // y
	engfunc(EngFunc_WriteCoord, originF[2] + -7.0) // z
	write_short( xSpriteFireExp );
	write_byte(random_num( 25, 40 ))
	write_byte(random_num( 8, 14 ))
	write_byte(TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND)
	...
	engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_EXPLOSION)
	engfunc(EngFunc_WriteCoord, originF[0] - 10.0) // x
	engfunc(EngFunc_WriteCoord, originF[1] - 40.0) // y
	engfunc(EngFunc_WriteCoord, originF[2] + 4.0) // z
	write_short( xSpriteFireSmoke );
	write_byte(random_num( 25, 40 ))
	write_byte(random_num( 8, 14 ))
	write_byte(TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND)
	message_end()
	...
	engfunc(EngFunc_WriteCoord, originF[0] + 5.0) // x
	engfunc(EngFunc_WriteCoord, originF[1] + 40.0) // y
	engfunc(EngFunc_WriteCoord, originF[2] + 1.0) // z
	write_short( xSpriteFireSmoke );
	write_byte(random_num( 25, 40 ))
	write_byte(random_num( 8, 14 ))
	write_byte(TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND)
	...
	// GIBS FLUTUANTES:
	static Float:d[3], Float:o[ 3 ];
	for( i = 0; i < random_num( 20, 25 ); i++ ){
		d[0] = random_float( -90.0, -2.0 )
		d[1] = random_float( 0.0, 360.0 )
		...
		engfunc(EngFunc_WriteCoord, originF[2] + random_float( 16.0, 120.0 )) // z
		engfunc(EngFunc_WriteCoord, o[0]) // x
		engfunc(EngFunc_WriteCoord, o[1]) // y
		engfunc(EngFunc_WriteCoord, o[2] + 4.0 ) // z
		write_short( xSpriteFireGib2 )
		write_byte( random_num( 5, 15 ) ) // tempo que os gibs irao flutuar
		write_byte(0)
		message_end()
	}
	
	for( i = 0; i < random_num(6, 10); i++ ){
		d[0] = random_float( -90.0, -2.0 )
		d[1] = random_float( 0.0, 360.0 )
		
		VelocityByAngle( d, random_float( 50.0, 100.0 ), o) 
	...
	for( i = 0; i < random_num(4, 12); i++ ){
		d[0] = random_float( -90.0, -2.0 )
		d[1] = random_float( 0.0, 360.0 )
		
		VelocityByAngle( d, random_float( 10.0, 60.0 ), o) 
...
stock CreateFrostBlast( ent, const Float:originF[3]){
	// Smallest ring Extra
	engfunc(EngFunc_MessageBegin, MSG_PVS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_BEAMCYLINDER) // TE id
	engfunc(EngFunc_WriteCoord, originF[0]) // x
	...
	engfunc(EngFunc_WriteCoord, originF[2] + 100.0) // z
	write_short( xSpriteFrostGib )
	write_byte(random_num(9,12)) // count
	write_byte(24) // life
	write_byte(2) // scale
	write_byte(20) // velocity randomness
	write_byte(15) // velocity randomness
	...
	for( i = 0; i < random_num( 20, 35 ); i++ ){
		...
		engfunc(EngFunc_WriteCoord, o[1]) // y
		engfunc(EngFunc_WriteCoord, o[2]) // z
		write_short( xSpriteFrostGib2 )
		write_byte( random_num( 6, 15 )) // tempo que os gibs irao flutuar
		write_byte(0)
		message_end()
	}
	
	/*
	new Float:o[3], Float:d[3], Float:o2[3], Float:d2, u
	for( a = 1; a < 33; a++ )
	{
		if( !is_user_connected(a) || !is_user_alive(a) || !zp_team( a, zombie )) continue
		
		
		pev( a, pev_origin, o2 )
		d2 = get_distance_f( originF, o2 );
		if( d2 < float( cfg_int(cfg_fire_radius, 0) ) )
		{
			if( zp_team( a, nemesis ) || zp_team( a, assassin ) || g_nodamage[a] )
			{
				static Float:origin2[3]
				pev( a, pev_origin, origin2 )
				zn_emit( cfg_frost_break, a,  CHAN_BODY,  1.0, ATTN_NORM, 0, PITCH_NORM )
				for( u = 0; u < random_num( 9, 14 ); u++ )
				{
					engmessage_begin2( MSG_PVS, SVC_TEMPENTITY, origin2, 0)
					write_byte(TE_BREAKMODEL) // TE id
					engfunc(EngFunc_WriteCoord,origin2[0]+random_float( -20.0, 20.0 )) // x
					engfunc(EngFunc_WriteCoord,origin2[1]+random_float( -20.0, 20.0 )) // y
					engfunc(EngFunc_WriteCoord,origin2[2]+random_float( 20.0, 60.0 )) // z
					...
					write_coord(random_num( 5, 10 )) // velocity z
					write_byte(10) // random velocity
					write_short(cfg_int(cfg_glass)) // model
					write_byte(5) // count
					write_byte(random_num( 20, 35 )) // life
					write_byte(BREAK_GLASS) // flags
					message_end()
			...
			{
				static Float:r
				r = random_float(cfg_float(cfg_freezeduration, 0), cfg_float(cfg_freezeduration, 1))
				f_unfreeze_time[a] = get_gametime() + r
				z_freezemax[a] = r
				continue
			}
			
			// Freeze icon?
			if (cfg_int(cfg_hudicons))
			{
				message_begin2(MSG_ONE_UNRELIABLE, g_msgDamage, _, a)
				write_byte(0) // damage save
				write_byte(0) // damage take
				write_long(DMG_DROWN) // damage type - DMG_FREEZE
				write_coord(0) // x
			...
			pev( a, pev_angles, f_player_angle[a] )
			pev( a, pev_v_angle, f_player_v_angle[a] )
			pev( a, pev_frame, f_player_frame[a] )
			f_player_anims[a][0] = pev( a, pev_sequence )
			f_player_anims[a][1] = pev( a, pev_gaitsequence )
			zn_create_icecube(o2, a)
			
			// Light blue glow while frozen
			zn_set_rendering(a, kRenderFxGlowShell, cfg_int(cfg_frozen_glow, 0), cfg_int(cfg_frozen_glow, 1), cfg_int(cfg_frozen_glow, 2), kRenderNormal, cfg_int(cfg_frozen_glow, 3))
			
			// Freeze sound
			zn_emit( cfg_frost_hit, a,  CHAN_BODY,  1.0, ATTN_NORM, 0, PITCH_NORM )
			
			// Add a blue tint to their screen
			message_begin2(MSG_ONE, g_msgScreenFade, _, a)
			write_short(0) // duration
			write_short(0) // hold time
			write_short(FFADE_STAYOUT) // fade type
			write_byte(0) // red
			...
			// Save player's old gravity (bugfix)
			pev(a, pev_gravity, g_frozen_gravity[a])
			
			// Prevent from jumping
			if (pev(a, pev_flags) & FL_ONGROUND)
			set_pev(a, pev_gravity, 999999.9) // set really high
			else
			set_pev(a, pev_gravity, 0.000001) // no gravity
			
			// Prevent from moving
			ExecuteHamB(Ham_Player_ResetMaxSpeed, a)
...
public register_dictionary_colored(const filename[])
{
	...
	while( !feof(fp) )
	{
		...
		else if( szBuffer[0] )
		{
			...
			{
				while( replace(szTranslation, charsmax(szTranslation), "!g", "^x04") ){}
				while( replace(szTranslation, charsmax(szTranslation), "!t", "^x03") ){}
				while( replace(szTranslation, charsmax(szTranslation), "!p", "^n") ){}
				while( replace(szTranslation, charsmax(szTranslation), "!n", "^x01") ){}
...
/*
public menu_roupas(id)
{
	if( get_user_flags(id) & ADMIN_RESERVATION )
	{
		menu_display(id, menuFANTASIA, 0)
	}
	else client_print(id, print_chat, "[ZM] Faca uma doacao ao servidor e pegue seu VIP !!!")
}

public menu_handlerFANTASIA(id, menu, item)
{
	if (item == MENU_EXIT)
	{
		return PLUGIN_HANDLED
	...
	switch(key)
	{
		...
		{	
			ModelVIP[id] = 0
			client_print(id, print_chat, "[ZM] Voce escolheu: VIP Branco - Original.")
			client_print(id, print_chat, "[ZM] Sua fantasia sera alterada no seu proximo respawn!")
		}
		case 2:
		{
			ModelVIP[id] = 1
			client_print(id, print_chat, "[ZM] Voce escolheu: Girl.")
			client_print(id, print_chat, "[ZM] Sua fantasia sera alterada no seu proximo respawn!")
		}		
		case 3:
		{
			ModelVIP[id] = 2
			client_print(id, print_chat, "[ZM] Voce escolheu: SPY (Team Fortress 2).")
			client_print(id, print_chat, "[ZM] Sua fantasia sera alterada no seu proximo respawn!")
		}
		case 4:
		{
			ModelVIP[id] = 3
			client_print(id, print_chat, "[ZM] Voce escolheu: Engineer (Team Fortress 2).")
			client_print(id, print_chat, "[ZM] Sua fantasia sera alterada no seu proximo respawn!")
		}
		case 5:
		{
			ModelVIP[id] = 4
			client_print(id, print_chat, "[ZM] Voce escolheu: Leon.")
			client_print(id, print_chat, "[ZM] Sua fantasia sera alterada no seu proximo respawn!")
		}	
	}
