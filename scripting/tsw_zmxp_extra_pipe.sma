...
new const g_vflare[] = "models/zp_plague_satelite/v_grenade_flare.mdl"; // type here your custom flare model to prevent bug. now uses default model
...
public plugin_init() {
	register_plugin("[ZMXP] Pipe Bomb", VERSION, "4eRT");
	register_forward(FM_SetModel,"fw_SetModel", 1);
	register_dictionary("play_zmxp_extra_pipe.txt");
	RegisterHam(Ham_Think, "grenade", "fw_ThinkGren");
	RegisterHam(Ham_Spawn, "player", "fw_Spawn");
	
	// Cvars
	cvar_duration = register_cvar("zp_pipe_duration", "8");
	cvar_radius = register_cvar ( "zp_pipe_radius", "300");
	cvar_enabled = register_cvar("zp_pipe_enabled", "1");
	cvar_cost = register_cvar("zp_pipe_cost", "28");
	cvar_mode = register_cvar("zp_pipe_mode", "1");
	cvar_sound = register_cvar("zp_pipe_sound", "1");
	cvar_hud = register_cvar("zp_pipe_hud", "1");
	cvar_damage = register_cvar("zp_pipe_damage", "1250");
	cvar_speed = register_cvar("zp_pipe_speed", "100.0");
	cvar_shake = register_cvar("zp_pipe_shake", "1");
	cvar_extraap = register_cvar("zp_pipe_ap", "1");
	cvar_Scost = register_cvar("zp_mshield_cost", "4");
	cvar_Senabled = register_cvar("zp_mshield_enabled", "0");
	cvar_Shud = register_cvar("zp_mshield_hud", "0");
...
public hook(entity) // Magnet func. Hooks zombies to nade
{
	...
	{
		if (!is_user_alive(victim) || !zp_get_user_zombie(victim) || zp_get_user_nemesis( victim ) || g_has_shield[victim])
			continue;
