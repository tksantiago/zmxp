...
{
	// New plugin
	register_plugin ( "[ZMXP] Granada de Alucinacoes", VERSION, "NiHiLaNTh" )
	
	// New extra item
	g_conc = zp_register_extra_item ( "Granada de Alucinacoes", GRENADE_COST, ZP_TEAM_ZOMBIE )
	...
	// CVARs
	cvar_nade_radius = register_cvar ( "zp_conc_nade_radius", "550" )
	cvar_duration = register_cvar ( "zp_conc_nade_duration", "10" )
...
public zp_extra_item_selected ( Player, Item )
{
	...
	if ( Item == g_conc )
	{
		if( comprou[Player] > 2)
		{
			client_print(Player, print_chat, "[ZP] Voce ja comprou 2 alucinacoes nesse round, espere o proximo!")
...
public fw_CmdStart ( Player, UC_Handle, Seed )
{
	...
	if ( !is_user_alive ( Player ) || zp_get_user_zombie ( Player ) /*|| zp_get_user_survivor( Player )*/ || !task_exists ( Player+TASK_AFFECT ) )
		return FMRES_IGNORED
