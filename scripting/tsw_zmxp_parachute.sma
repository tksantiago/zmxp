...
#define _PLUGIN         "[ZMXP] Parachute"
#define _VERSION             "1.0"
#define _AUTHOR           "H.RED.ZONE"

#define PARACHUTE_MODEL "models/csp_parachute.mdl"
...
{
	if(gamemode == MODE_NEMESIS || gamemode == MODE_PLAGUE || gamemode == MODE_SURVIVOR)
	{
		for (new x = 1; x <= gServerMaxPlayers; x++) 
		{
			if( HasUserParachute(x) && is_user_alive(x) )
			{
				new iEnt = g_iUserParachute[x]
				if( iEnt ) {
					RemoveUserParachute(x, iEnt)
				}
				ClearUserHasParachute(x)
			}
		}
	}
}
...
CreateParachute(id) {
	...
	/*if ( !pev_valid(iEnt) ) // nao eh valido
	{
		return 0
	}*/
...
public fw_Start(id) {
	if( ~HasUserParachute(id) || !is_user_alive(id) || !is_user_admin(id) ) {
		return
	}
	
	if( zp_is_survivor_round() || zp_is_plague_round() || zp_is_nemesis_round() ) {
		return
	}
