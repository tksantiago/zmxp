...
new cvar_voteMapChoiceCnt, cvar_voteAnnounceChoice, cvar_voteUniquePrefixes, cvar_rtvReminder;
new cvar_srvStart;
...
//new cvar_map_refem, cvar_map_bomba, cvar_map_awp, cvar_map_facas, cvar_map_bandeira;

native GetModRunning();
native GetNextMod();
native SetNextMod( mod );
...
public plugin_init(){
	...
	parse( VERSION, version, sizeof(version)-1, jnk, sizeof(jnk)-1, rev, sizeof(rev)-1, jnk, sizeof(jnk)-1);
	...
	register_dictionary("tsw_votemaps.txt");
	...
	register_clcmd("listmods", "ListMods");
	...
	//cvar_map_refem = register_cvar("gal_vote_mapfile1", "addons/amxmodx/configs/tsw_votemaps/tsw_maps-refem.ini");
	//cvar_map_bomba = register_cvar("gal_vote_mapfile2", "addons/amxmodx/configs/tsw_votemaps/tsw_maps-bomba.ini");
	//cvar_map_awp = register_cvar("gal_vote_mapfile3", "addons/amxmodx/configs/tsw_votemaps/tsw_maps-awp.ini");
	//cvar_map_facas = register_cvar("gal_vote_mapfile4", "addons/amxmodx/configs/tsw_votemaps/tsw_maps-facas.ini");
	//cvar_map_bandeira = register_cvar("gal_vote_mapfile5", "addons/amxmodx/configs/tsw_votemaps/tsw_maps-bandeira.ini");
...
public plugin_natives(){
	register_native( "set_start_vote_map", "native_start_vote_map", 1 );
}

public native_start_vote_map(){
...
public plugin_cfg()
{
	formatex(DIR_CONFIGS[get_configsdir(DIR_CONFIGS, sizeof(DIR_CONFIGS)-1)], sizeof(DIR_CONFIGS)-1, "/tsw_votemaps");
	formatex(DIR_DATA[get_datadir(DIR_DATA, sizeof(DIR_DATA)-1)], sizeof(DIR_DATA)-1, "/tsw_votemaps");

	server_cmd("exec %s/tsw_votemaps.cfg", DIR_CONFIGS);
	server_exec();
...
//	g_nonOverlapHudSync = CreateHudSyncObj();
	...
	if (get_pcvar_num(cvar_emptyWait)){
		...
		log_message("teste mapas favoritos 1")
	}
}
...
public vote_setupEnd(){
	...
	if( get_pcvar_num( cvar_endOfMapVote ))
		formatex( iNextMap, charsmax( iNextMap ), "[Ainda nao Votado]");
	
	else {
		xMapsFavoritos = ArrayCreate( 32 );
		map_populateList( xMapsFavoritos, "addons/amxmodx/configs/tsw_votemaps/tsw_maps-favoritos.ini");
		map_getNext( xMapsFavoritos, g_currentMap, iNextMap );
	}
	
	map_setNext( iNextMap );
...
public cmd_nextmap( id ){
	...
	//ColorChat( 0, print_chat, "Proximo Mapa sera %s", map);
	ColorChat( id, GREEN, "^x01 Proximo mapa sera:^x03 %s^x01 no mod^x03 %s^x01!", map, TSWModsNames[ GetModRunning() ]);
	return PLUGIN_CONTINUE;
}
...
public map_populateList(Array:mapArray, mapFilename[]){
	...
	if( !equal( mapFilename, "*")){
		...
		if( file ){
			...
			while( !feof( file )){
				...
				log_message("Carrego a Lista de mapas favoritos: %s", mapFilename );
			}
			
			fclose(file);
		}
		
		else log_error(AMX_ERR_NOTFOUND, "%L", LANG_SERVER, "GAL_MAPS_FILEMISSING", mapFilename);
	}
...
public map_loadNominationList(){
	new filename[ 256 ];
	switch( GetModRunning()){
		case 0: formatex( filename, charsmax( filename ), "addons/amxmodx/configs/tsw_votemaps/tsw_maps-refem.ini"); //get_pcvar_string( cvar_map_refem, filename, sizeof( filename )-1);
		case 1: formatex( filename, charsmax( filename ), "addons/amxmodx/configs/tsw_votemaps/tsw_maps-bomba.ini"); //get_pcvar_string( cvar_map_bomba, filename, sizeof( filename )-1);
		case 2: formatex( filename, charsmax( filename ), "addons/amxmodx/configs/tsw_votemaps/tsw_maps-awp.ini"); //get_pcvar_string( cvar_map_awp, filename, sizeof( filename )-1);
		case 3: formatex( filename, charsmax( filename ), "addons/amxmodx/configs/tsw_votemaps/tsw_maps-facas.ini"); //get_pcvar_string( cvar_map_facas, filename, sizeof( filename )-1);
		case 4: formatex( filename, charsmax( filename ), "addons/amxmodx/configs/tsw_votemaps/tsw_maps-bandeira.ini"); //get_pcvar_string( cvar_map_bandeira, filename, sizeof( filename )-1);
	}
...
public cmd_createMapFile(id, level, cid)
{
	...
	switch (cntArg)
	{
		...
		default:
		{
			...
			con_print(id, "Uso: gal_createmapfile <arquivo>");
			con_print(id, "O arquivo sera criado em ./amxmodx/configs/tsw_votemap");
		}
	}		
	return PLUGIN_HANDLED;
}
...
{
	new filename[256];
	formatex(filename, sizeof(filename)-1, "%s/tsw_prefixes.ini", DIR_CONFIGS);
...
public map_loadEmptyCycleList(){
	new filename[ 256 ];
	formatex( filename, charsmax( filename ), "addons/amxmodx/configs/tsw_votemaps/tsw_maps-favoritos.ini");
	
	log_message(" 1 - Carrego favoritos: %s", filename );
	//new filename[256];
...
public map_manageEnd(){
	...
	// Verificamos se o servidor tiver menos de 2 players
	if( get_realplayersnum() <= 1 ){
		...
		if (get_pcvar_num(cvar_endOnRound) && g_wasLastRound == false)
		{
			...
			{
				client_print(0, print_chat, "Mudanca de mapa ao fim dessa rodada.");
			}
			else
			{
				client_print(0, print_chat, "O Limite de Tempo expirou. Mudanca de mapa ao fim dessa rodada.");
			}
...
nomination_attempt(id, nomination[]) // (playerName[], &phraseIdx, matchingSegment[])
{
	...
	g_nominationMatchesMenu[id] = MenuCreate("Selecione um Mapa", "nomination_handleMatchChoice");
	...
	for (mapIdx = 0; mapIdx < g_nominationMapCnt && matchCnt <= MAX_NOM_MATCH_CNT; ++mapIdx)
	{
		...
		if (contain(nominationMap, nomination) > -1)
		{
			...
			{
				formatex(disabledReason, sizeof(disabledReason)-1, "\d[Sugerido]");
			}
			// disable if the map is too recent
			else if (map_isTooRecent(nominationMap))
			{
				formatex(disabledReason, sizeof(disabledReason)-1, "\d[Muito Recente]");
			}
			else if (equal(g_currentMap, nominationMap))
			{
				formatex(disabledReason, sizeof(disabledReason)-1, "\d[Mapa Atual]");
			}

			formatex(choice, sizeof(choice)-1, "\y%s %s", nominationMap, disabledReason);
			menu_additem(g_nominationMatchesMenu[id], choice, info, (disabledReason[0] == 0) ? 0 : (1<<26));
		}
	}
	
	switch( matchCnt ){
		case 0: ColorChat( id, GREEN, "^x01 O Mapa^x04 %s^x01 nao corresponde ao mod atual!", nomination);
		case 1: map_nominate(id, matchIdx);
...
nomination_cancel(id, idxMap)
{
	...
	if (g_voteStatus & VOTE_IN_PROGRESS)
	{
		client_print(id, print_chat, "Cancelamento falhou; votacao ja esta em progresso.");
		return;
	}
	// and if the outcome of the vote hasn't already been determined
	else if (g_voteStatus & VOTE_IS_OVER)
	{
		client_print(id, print_chat, "Cancelamento falhou; o resultado da votacao ja foi determinado.");
		return;
	}
	...
	else
	{
		...
		if (idNominator)
		{
			...
			client_print(id, print_chat, "Cancelamento falhou; %s foi sugerido por %s.", mapName, name);
		}
		else
		{
			client_print(id, print_chat, "Cancelamento falhou; you have not previously nominated %s.", mapName);
		}
	}
}
...
vote_addFiller( mod ){
	...
	//client_print(0, print_chat, "Mod: %s", TSWModsNames[ mod ]);
...
public vote_display(arg[3])
{
	...
	if (g_refreshVoteStatus || isVoteOver)
	{
		...
		{
			charCnt = formatex(voteStatus, sizeof(voteStatus)-1, "\d%s^n%s - Escolha o Mapa^n%s^n^n\rProximo Mod sera: %s^n", xMenuLine, xPrefix, xMenuLine, TSWModsNames[ GetNextMod() ] );
		}

		// add maps to the menu
		// aki fica o nome dos mapas
		for (new choiceIdx = 0; choiceIdx < g_choiceCnt; ++choiceIdx)
		{
			...
			charCnt += formatex(voteStatus[charCnt], sizeof(voteStatus)-1-charCnt, "^n\r%i. \y%s%s", choiceIdx+1, g_mapChoice[choiceIdx], voteTally );
			keys |= (1<<choiceIdx);
			...
			charCnt += formatex(voteStatus[charCnt], sizeof(voteStatus)-1-charCnt, "^n%s%i. %s%s%s - pika 3", CLR_RED, choiceIdx+1, CLR_WHITE, g_mapChoice[choiceIdx], voteTally);
			keys |= (1<<choiceIdx);
			*/
		}
		...
		if (g_vote[0] == 0)
		{
			...
			formatex(g_vote[cleanCharCnt], sizeof(g_vote)-1-cleanCharCnt, "^n^n%s0. %s Nenhum(a)", CLR_RED, CLR_WHITE );
		}
...
*/
		...
		if (idxWinner == g_choiceCnt)
		{
			...
			{
				new nextMap[32];
				formatex(nextMap, sizeof(nextMap)-1, "[Ainda nao Votado]");
				map_setNext(nextMap);
			}
		...
		else 
		{
			...
			format(conffile,199,"%s/maps_sem_biohazard.ini",configdir);
			...
			for(new i = 0 ; i <= lines; i++)
			{
				...
				if(equali(file, g_mapChoice[idxWinner]))
				{
					server_cmd("zp_nobiohazardmod"); // sem mod de bio pra esse mapa!
					break;
				}
			}
			*/
			...
			// Começar vote de mod normal... se for permitido =)
			//set_task(1.0, "votamod");
			
			client_cmd( 0, "spk attention");
			
			set_hudmessage( 255, 0, 0, -1.0, -1.0, 1, 6.0, 5.0, 0.1, 0.1, -1);
			show_hudmessage(0, "O Proximo Mapa sera %s^n Modo %s!", g_mapChoice[idxWinner], TSWModsNames[ GetNextMod() ]);
			
			ColorChat( 0, GREEN, "^x03 Votacao finalizada, o proximo mapa sera^x04 %s^x01 no modo^x04 %s^x01!", g_mapChoice[idxWinner], TSWModsNames[ GetNextMod() ]);
...
public vote_handleChoice(id, key)
{
	...
	if (g_voted[id] == false)
	{
		...
		if (key == 9)
		{
			...
			if (get_pcvar_num(cvar_voteAnnounceChoice))
			{
				client_print(0, print_chat, "O Player %s escolheu nao participar da votacao.", name);
			}
			else
			{
				client_print(id, print_chat, "Voce escolheu nao participar da votacao.");
			}
		}
		else
		{
			...
			if (key == g_choiceCnt)
			{
				...
				if (g_voted[id] == false)
				{
					...
					if (get_pcvar_num(cvar_voteAnnounceChoice))
					{
						client_print(0, print_chat, "%s escolheu extender o mapa atual", name);
					}
					else
					{
						client_print(id, print_chat, "Voce escolheu extender o mapa atual.");
					}
				}
			}
			else
			{
				...
				if (get_pcvar_num(cvar_voteAnnounceChoice))
				{
					client_print(0, print_chat, "O Player %s escolheu %s.", name, g_mapChoice[key]);
				}
				else
				{
					client_print(id, print_chat, "Voce escolheu %s.", g_mapChoice[key]);
				}
			}
...
public vote_rock(id)
{
	...
	/* PAUSADO888888888888888888888888888888888888888888888888888888888888888888888888888 */
	if (g_rtvWait){
	...
	if (++g_rockedVoteCnt >= rocksNeeded)
	{
		...
		server_cmd("tsw_votemap");
		
		// start up the vote director 
		//vote_startDirector(true); pausado para testesss
	}
	else
	{
...
public ListMods( id ){
	con_print( id, "^n[%s] Mods Sugeriveis:", xPrefix );
	con_print( id, "*********************************************");
	
	new iCount = 1;
	for( new i = 0; i < sizeof( TSWModsNames ); i++ ){
		con_print( id, "%d: %s", iCount, TSWModsNames[ i ]);
		iCount++;
	}
...
map_listAll(id)
{
	...
	con_print( id, "^n[%s] Mapas Sugeriveis para o Mod %s:", xPrefix, TSWModsNames[ GetModRunning() ], g_nominationMapCnt );
	con_print( id, "*********************************************");
	...
	if( mapCount && mapCount < g_nominationMapCnt ){
		con_print( id, "*********************************************");
		con_print( id, "Mapas %i - %i de %i", start, idx, g_nominationMapCnt );

		if( end < g_nominationMapCnt )
			con_print( id, "Use '%s %i' ou '%s *' para ver Mais mapas", command, end + 1, command);
...
{
	client_print(0, print_chat, "Os seguintes mapas foram excluidos da lista de sugestoes: %s", nominations);
}
